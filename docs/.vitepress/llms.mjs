 ...
 *
 * The site is hosting-neutral (no hostname, no `base`, no clean URLs), so every link written here
 * is relative to the site root and resolves wherever the build is served. Each twin sits at its
 * source path (`quickstart.md` → `/quickstart.md`), which keeps the relative `.md` links between
 * pages pointing at the neighbouring twins.
 */
...
/** The rendered page for a source path, as VitePress writes it without clean URLs. */
const htmlFileFor = (relativePath) =>
  relativePath === HOME ? './' : relativePath.replace(/(^|\/)index\.md$/, '$1').replace(/\.md$/, '.html')
...
/** `/llms.txt` — the index. */
function renderIndex({ version, repository, sidebar }) {
  const lines = [
    ...header(version, repository),
    'Every page below, concatenated as one document: llms-full.txt',
    '',
  ...
  for (const section of group(sidebar)) {
    ...
    for (const page of section.items) {
      ...
      lines.push(`- [${page.title}](${page.relativePath})${description}`)
    }
    lines.push('')
  }
...
async function renderFull({ version, repository, sidebar, srcDir }) {
  const parts = [
    ...header(version, repository),
    'The complete text of the DecGuard documentation, generated at build time. Index: llms.txt',
    '',
  ]
  ...
  for (const section of group(sidebar)) {
    for (const page of section.items) {
      ...
      parts.push('', heading, '', `Source: ${page.relativePath}`, '', rest.join('\n').trim(), '')
    }
  }
 ...
 */
async function writePageFiles({ outDir, srcDir, index, siteDescription }) {
  for (const page of pages.values()) {
    ...
    const document = [
      ...
      `source: ${htmlFileFor(page.relativePath)}`,
      '---',
      '',
      body,
      '',
...
export async function writeLlmsFiles({ outDir, srcDir, version, repository, sidebar, siteDescription }) {
  const index = renderIndex({ version, repository, sidebar })
  const full = await renderFull({ version, repository, sidebar, srcDir })
  ...
  const twins = await writePageFiles({ outDir, srcDir, index, siteDescription })
